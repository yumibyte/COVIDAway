...
class Individual(models.Model):
    ...
    def __str__(self):
        return str([self.time.strftime("%m/%d/%Y - %H:%M"), self.name, self.email, self.people, [self.need_gloves, self.need_faceshields, self.need_facemasks], self.notes, self.long, self.lat])
